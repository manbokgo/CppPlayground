...
// 보석무게 내림차순, 가방무게 내림차순으로 정렬하여
// '보석을 순회'하며 가방에 무거운 보석부터 채울 수 있을만큼 채운다.

// 만약 현재 보석 무게가 커서 현재 가방엔 넣을 순 없거나, 가방이 모두 꽉 찬 상황에서
// 기존에 넣었던 '최솟값' 가치의 보석보다 현재 보석 가치가 더 크다면, 이전 가방 속 보석을 대체하는 식.
// 보석무게가 내림차순이니 현재 보석은 반드시 이전 가방에 들어갈 수 있음.

// 그런데 로직상 더 직관적인 것은
// 현재 가방에 들어갈 수 있는 보석 후보는 현재 가방무게 이하인 보석들이라는 것을 이용한다.

// 보석무게 오름차순, 가방무게 오름차순으로 정렬하여
// '가방을 순회'하며 현재 가방무게 이하인 보석들을 몽땅 우선순위 큐에 넣고
// 그 중 '최댓값'을 뽑아서 현재 가방에 넣고 하는 식임

// 이렇게 간단한 걸 왜 진작 생각 못 했지...
// 그나마 내 풀이는 '채울 수 있을만큼 채운다' 과정이 우선순위 큐에서 최댓값을 뽑는 것이 아니기 때문에
...
int main()
{
    ...
    // 보석무게, 가방무게 오름차순. 가방 순회. 최댓값 뽑기. 직관적풀이 184ms
    ...
    // 보석무게, 가방무게 내림차순. 보석 순회. 최솟값 대체하기. 제출풀이 132ms
    /*
