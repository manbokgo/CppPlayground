...
dx, dy의 값을 더하면서, OOB로 경계 넘거나 벽 만나면 멈춰서
그동안 몇 타일을 밝혀냈는지 세는 방식이 깔끔하다. 예전엔 이렇게 했는데 왜 기억을 못 했는지.

난 DFS로 백트래킹을 진행했는데 4진법으로
각 CCTV의 4가지 방향을 조합한 모든 경우의 수를 구하는 방법이 참 세련됨. 본받자.
