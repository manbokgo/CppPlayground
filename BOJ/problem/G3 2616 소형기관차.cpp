...
왼쪽구간최댓값 + mid + 오른쪽구간최댓값  의 최댓값으로 정답을 구하는 풀이. 인덱스 넣는 게 복잡함.

왼쪽 구간의 최댓값: mid가 한 칸씩 움직일 때마다 왼쪽 구간의 오른끝에 열차 배치 가능한 위치가 하나씩 늘어나므로
그 위치에 열차를 둘 때의 운송값이 기존 왼쪽구간 최댓값보다 큰 지 체크하고 (따지고 보면 DP 배열 계산하는 방식이다)

오른쪽 구간의 최댓값: 무식하게 오른쪽 구간을 O(n) 전체 순회해서 최댓값을 찾고, 무효화될 때마다 다시 찾는 식으로 푸니
108ms로 정답. (std::max가 아니라 std::max_element 써야하는데 실수해서 오래 삽질함)
하지만 승객수 내림차순이라는 최악 케이스에서는 O(n^2)이므로 시간초과 나는게 맞는데 채점 테케가 부족한 듯하다.

+
왼쪽 구간의 최댓값: dpLeft[i] 1 ~ i 구간에서의 최댓값
오른쪽 구간의 최댓값: dpRight[i]  i ~ n 구간에서의 최댓값

위에서 왼쪽 구간의 최댓값을 구하는 방식대로 dpLeft를 미리 저장해두고,
유사한 방식으로 --i 루프를 통해 오른쪽 구간의 최댓값 dpRight를 미리 저장해두어 써먹음. 4ms 정답.

+
이 정도로(특히 인덱스 쪽) 복잡하게 풀어야하는 문제는 아닌 거 같아서 생각 비우고 순수하게 DP 바텀업으로 생각해봄.
'어떤 구간의 최댓값'이 핵심 키워드라는 걸 눈치채고 30분 생각해서 DP 점화식 깨달음. 4ms 정답. 끼얏호우~
...
// mid 열차 왼->오로 옮기면서 좌우 구간 최댓값 구하기
int main()
{
    ios::sync_with_stdio(false);
    cin.tie(nullptr);
    ...
    vector<int> dpLeft(n + 1); // 1 ~ i 구간에서의 최댓값
    for (int i = len; i <= n; ++i)
    {
        dpLeft[i] = max(dpLeft[i - 1], sum[i - len + 1]);
    }

    vector<int> dpRight(n + 2); // i ~ n 구간에서의 최댓값
