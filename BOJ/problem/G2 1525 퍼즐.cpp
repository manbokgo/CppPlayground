...
// 9칸 배열의 상태로 해시해야 해서 1차원 배열로 두어야 했는데,
// 1차원 인덱스 dx +1, -1, +3, -3 으로 동서남북 탐색이 가능하다고 잘못 생각함
// 좌표상 빈칸이 맨오른쪽에 있으면(2열) +1 동쪽 탐색이 불가능한데 이대로 하면 잘못 진행된다
// 똑같은 거 예전에도 당했던 거 같은데 반성하자...

// 동서남북 탐색만 2차원 좌표로 계산하는 것으로 바꿔서 해결
