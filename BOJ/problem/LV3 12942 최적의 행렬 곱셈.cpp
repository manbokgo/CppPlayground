...
// 브루트 포스는 안 될 거라고 짐작하고 넘김. 정확하진 않지만 먼저 곱할 것을 순열로 뽑는다치면 n! 정도라고 추정함.
...
// (i*...*k)*(k+1*...*j)    DP[i][k] + DP[k+1][j] + i행*k열*j열 (i<=k<j)의 최소값이다. i=j이면 연산 수는 0.

// 시간복잡도는 O(n^3).
// 위키백과 링크에 따르면 O(n log n) 알고리즘도 존재하고, 깃헙에 C++ 샘플도 존재하긴 한데... 더 깊게 파진 않음.

// 탑다운으로 구현했는데, 바텀업으로 2차원 배열의 대각선 줄을 차근차근 채워가면서도 구현할 수 있음
// DP에 저장되는 최소값일 때의 k를 따로 배열로 저장해서, 실제 괄호를 어디에 쳐야하는지도 복구할 수 있음
