...
// 예시가 너무나도 JPS, A* 길찾기 알고리즘 연상되어서 주화입마 온 듯.
// BFS 돌리면 도달 불가능한 경로까지 계산하잖아? 연산 줄이려면 휴리스틱 써야겠네? 하면서
// 처음엔 휴리스틱 만들어서 도착지로 가는 가장 짧은 경로상의 타일만 택하되,
// 현재 움직이고 있는 방향과 지금까지 꺾인 횟수를 아는 상태에서 현재 타일의 앞,좌,우 3방향을 pq에 넣고,
// pq에서 가장 낮은 꺾인횟수 값을 가지고 있는 앞 타일에 먼저 방문해서 다시 3방향을 pq에 넣고, 한 칸씩 한 칸씩 반복하는...
//
// 그런 요상한 방법으로 한 30분 생각했는데 너무 복잡한 거 같아서 그림판 띄워놓고 곰곰히 생각해보니
// 근본적으로 문제의 요점은 선분들의 개수를 세는 것이고, 어떤 타일을 첫 방문할 때의 선분이 최소 선분 수인만큼 방문 후엔 스킵 가능하니
// 선분을 가능한 끝까지 쭉쭉 그리는, 시작점으로부터 선분 1개, 2개, 3개.. 단계로 BFS 돌리는게 그냥 간단함을 깨닫고 풀음
//
// + 맨처음 구상대로 구현해봄. 예상한대로 생각하기 까다로운 부분들이 많아서 실수 와장창하다가 제대로 된 구현하는데 1시간 걸림
//
// 일단 휴리스틱은 애당초 잘못된 시도. 문제 의도 자체가 거리는 좀 더 멀더라도 선분 수는 더 적은 경우를 고려하는 거다. 바보임.
// 막연히 이게 없으면 연산이 불어날 거 같아서 넣은건데, 다시 생각해보니 겨우 100x100 타일 밖에 없다..
// 난 게임 알고리즘 구현을 하는게 아니라 PS를 하고 있는 거야.
//
...
//
// 저격 케이스가 있다. 위처럼 →,↑처럼 서로 방향이 다른게 교차하는 경우면 별 문제가 없으나
// 이 케이스처럼 ↓,↑ 마주 보는 방향으로 교차하면 중복으로 pq에 들어가는 문제가 생김.
// [1,2]칸이 pq에 중복으로 2개 들어가고, 그 둘이 똑같이 [1,3]을 2개 추가하는 것이 수십번 반복되면서
